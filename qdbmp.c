...
BMP* BMP_ReadFile( const char* filename )
{
	...
	/* Read image data */
	int bytes_read = 0;
	if ( (bytes_read = fread( bmp->Data, sizeof( UCHAR ), bmp->Header.ImageDataSize, f )) != bmp->Header.ImageDataSize )
	{
		BMP_LAST_ERROR_CODE = BMP_FILE_INVALID;
		printf("Meant to read %lu bytes but only read %d\n", bmp->Header.ImageDataSize, bytes_read);
		fclose( f );
		free( bmp->Data );
		free( bmp->Palette );
		free( bmp );
